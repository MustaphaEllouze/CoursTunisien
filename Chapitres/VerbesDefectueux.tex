...
En \textbf{tunisien}, le principe reste le même pour ces trois consonnes, cependant il me semble intéressant de mentionner ce qui suit: la plupart des verbes \textbf{arabe} comportant un \textbf{/'/} sont devenus \textit{en partie} défectueux puisque le coup de glotte disparaît au début et à la fin de tous les mots. On notera par exemple \textbf{/'axa\dh a/} \RL{أخذ} (\textit{prendre}) qui est devenu \textbf{/x\dh èè/}. Dans la suite, j'analyserai donc ces verbes comme tels, bien qu'ils ne soient pas défectueux en \textbf{arabe}.
...
Il n'y a priori pas de moyen de savoir si un verbe fait partie d'un groupe ou de l'autre, vous n'aurez donc le choix que d'apprendre par c\oe ur pour chacun des verbes. 
...
\begin{itemize}
    ...
    \item Les verbes qui se conjuguent avec un \textbf{/a/} au présent, qui dérivent de l'\textbf{arabe} de verbes se terminant par \textbf{/'/} : \textbf{qraa - yaqraa} (\textit{lire/étudier},  de \RL{قرأ يقرأ}) ; 
